...
resource "google_compute_instance" "compute_instance" {
  ...
  boot_disk {
    initialize_params {
      ...
      size  = each.value.boot_disk_initialize_params_size
    }
  }
...
# resource "google_compute_instance_group" "faz-umigs" {
...
#   region  = var.region
#   network = google_compute_subnetwork.compute_subnetwork["fazha-subnet-1"].network
# }
...
#   subnetwork {
#     name                    = google_compute_subnetwork.compute_subnetwork["fazha-subnet-1"].self_link
...
#   name         = "${var.prefix}-rt-default-via-faz-${random_string.string.result}"
